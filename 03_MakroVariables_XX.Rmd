...
-> Check y-axis ticks for plots! (see, e.g., GDP nominal)
...
ggsave(plot=plot, width=10, height=6, dpi=300, filename = "./figures/US_GDP_02.svg")
...
ggsave(plot=plot, width=10, height=6, dpi=300, filename = "./figures/US_GDP_03.svg")
...
ggsave(plot=plot, width=10, height=6, dpi=300, filename = "./figures/US_GDP_04.svg")
...
# Data
ser.dat <- get_eurostat(id = "namq_10_gdp",
                        time_format = "date",
...
# Transform date to yearqtr and date class
ser.dat$date.zoo <- as.yearqtr(as.character(ser.dat$time), "%Y-Q%q")
ser.dat$date.dat <- as.Date(ser.dat$date.zoo, format = "%Y Q%q")

# Subset data
ser.dat <- subset(ser.dat, date.dat %in% seq(as.Date("2000-01-01"), as.Date("2023-01-01"), "quarter"))

# Construct plot
ggplot(data = ser.dat, mapping = aes(x = date.zoo, y = values, color = geo)) +
  geom_line(size = 1) +
  scale_y_continuous(labels = comma_format(big.mark = ".", decimal.mark = ",")) +
  ...
  labs(title = "Reales Bruttoinlandsprodukt (BIP)",
       ...
       x = "", y = "Verkettete 2005 EUR in Millionen") +
  mod.theme.01
...
tmp <- eurostat_eurostat_json_url_edit_xx(id = "namq_10_gdp", filters = list(geo = c("DE","FR","EA19"),
                                                                             na_item = c("B1G"),
                                                                             unit = c("CLV05_MEUR"),
                                                                             s_adj = c("SCA")),
                                                                             lang = NULL)
...
# See: https://wikis.ec.europa.eu/display/EUROSTATHELP/API+Statistics+-+data+query
# see: https://ec.europa.eu/eurostat/web/query-builder/tool
...
tab <- cbind(tail(as.yearqtr(ser.dat[ser.dat$geo %in% c("DE"),]$date.zoo), 10),
             tail(ser.dat[ser.dat$geo %in% c("DE"), c(6)], 10),
             tail(ser.dat[ser.dat$geo %in% c("FR"), c(6)], 10),
             tail(ser.dat[ser.dat$geo %in% c("EA19"), c(6)], 10))
...
ggsave(plot=plot,width=10,height=6,dpi=300,filename = "./figures/US_CPI.svg")
...
# Data
ser.dat <- get_eurostat(id = "prc_hicp_manr",
                        filters = list(geo = c("DE","FR","EA19"),
                                     sinceTimePeriod = c("2010M01"),
                                     unit = c("RCH_A"),
                                     coicop = c("CP00")))

# Transform date to yearmon and date class
ser.dat$date.zoo <- as.yearmon(as.character(ser.dat$time), "%Y-%m")
ser.dat$date.dat <- as.Date(ser.dat$date.zoo, format = "%b %Y")

# Subset data
ser.dat <- subset(ser.dat, date.dat %in% seq(as.Date("2000-01-01"), as.Date("2023-02-01"), "month"))
...
plot <- ggplot(data = ser.dat, mapping = aes(x = date.zoo, y = values, color = geo)) +
  ...
  labs(title = "HVPI (jährliche Veränderungrate)",
       subtitle = paste0(c("(Letzter Beobachtungswert: "), ser.dat$date.zoo[length(ser.dat$date.zoo)], c(")")),
       caption = "Quelle: Eurostat", 
       x = "", y = "Prozent") +
  mod.theme.01
...
tab <- cbind(tail(ser.dat[ser.dat$geo %in% "DE", 6], 10),
             tail(ser.dat[ser.dat$geo %in% c("DE"), 5], 10),
             tail(ser.dat[ser.dat$geo %in% c("FR"), 5], 10),
             tail(ser.dat[ser.dat$geo %in% c("EA19"), 5], 10))
...
ggsave(plot=plot,width=10,height=6,dpi=300,filename = "./figures/EURO_CPI.svg")

```

```{r}

# source("./Rscripts/r_eurostat_helper_functions.R")
...
ggsave(plot=plot,width=10,height=6,dpi=300,filename = "./figures/US_FEDFUNDS.svg")
...
ggsave(plot=plot,width=10,height=6,dpi=300,filename = "./figures/EURO_MLF.svg")
...
ggsave(plot=plot,width=10,height=6,dpi=300,filename = "./figures/EURO_MRO.svg")
...
# See: 
# See: https://wikis.ec.europa.eu/display/EUROSTATHELP/API+Statistics+-+data+query
# See: https://ec.europa.eu/eurostat/web/query-builder/getting-started/query-builder
...
ggsave(plot = plot, width = 10, height = 6, dpi = 300, filename = "./figures/INT_RIS.svg")
...
ggsave(plot=plot,width=10,height=6,dpi=300,filename = "./figures/INT_MAT.svg")
...
ggsave(plot = plot, width = 10, height = 6, dpi = 300, filename = "./figures/FX_EURUSD.svg")
...
ggsave(plot = plot, width = 10, height = 6, dpi = 300, filename = "./figures/FX_USDJPY.svg")
...
ggsave(plot = plot, width = 10, height = 6, dpi = 300, filename = "./figures/FX_UKPUSD.svg")
